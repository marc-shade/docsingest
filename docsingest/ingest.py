...
def _ensure_nltk_data():
    """Download NLTK resources on first use, not at import time."""
    ...
    try:
        ...
        logging.warning("Could not download NLTK resources: %s", e)
    _nltk_downloaded = True

from docsingest.tree_generator import generate_document_tree

# Try importing PIIDetector
try:
    from docsingest.pii_detector import PIIDetector
except ImportError:
    logging.warning("PIIDetector could not be imported")
    PIIDetector = None
...
def _write_compliance_report(report_path: str, compliance_results: Dict) -> None:
    ...
    try:
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write("Generated by docsingest\n\n")
            f.write("---\n\n")
...
def ingest_documents(
    ...
    for root, dirs, files in os.walk(input_directory):
        # Apply ignore patterns (glob-style via fnmatch)
        dirs[:] = [d for d in dirs if not any(
            fnmatch.fnmatch(d, pattern) or fnmatch.fnmatch(os.path.join(root, d), pattern)
            for pattern in ignore_patterns
        )]
        files = [f for f in files if not any(
            fnmatch.fnmatch(f, pattern) or fnmatch.fnmatch(os.path.join(root, f), pattern)
            for pattern in ignore_patterns
        )]
        ...
        for filename in files:
            ...
            try:
                ...
                if text_extractor:
                    ...
                    # Tokenize content using tiktoken for accuracy
                    try:
                        import tiktoken
...
def extract_text_from_markdown(file_path):
    """Extract text from .md files as plain text (strips markdown formatting)."""
    try:
...
def _extract_text_from_txt(file_path):
    """Extract text from .txt files with proper resource management."""
    try:
